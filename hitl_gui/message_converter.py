...
def component_status(snapshot: dict, ready_age: float, warning_age: float) -> dict:
    joint_status, joint_age = heartbeat_status(snapshot.get("joint_last_time"), ready_age, warning_age)
    camera_last = max(filter(None, [snapshot.get("rgb_last_time"), snapshot.get("depth_last_time")]), default=None)
    camera_status, camera_age = heartbeat_status(camera_last, ready_age, warning_age)
    gripper_status, gripper_age = heartbeat_status(snapshot.get("gripper_last_time"), ready_age, warning_age)
    nodes = set(snapshot.get("node_names", []))
    services = set(snapshot.get("service_names", []))
    return {
        "UR5": {"status": joint_status, "last_message_age": joint_age, "joint_count": snapshot.get("joint_count", 0)},
